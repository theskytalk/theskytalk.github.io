    ...
    if (!navigator.serviceWorker.controller || !localStorage.SkyApp) {
        ...
        navigator.serviceWorker.addEventListener('message', e => {
            ...
            if (req === "load") {
                ...
                console.log('north got push');
                SWMSG = data;
                if(data.includes('recon')){
                    console.log('Recon request')
                    console.log('with data - \n');

                    let dddd = JSON.parse(data);
                    details_a = JSON.parse(dddd.body);
                    console.log(details_a);
                    role = 'B';
                    d = details_a.reg_offer;
...
const nav = (p) => {
    ...
    try { pathEvents(cr); initAppCore(); } catch (e) { console.warn('pathEvents not declared'); }
