...
const force_upgrade_string = "Let there be lights";
const sr_version = "2.1.1";
let AppCache = [];
let AppManifest;
let NewAppManifest;
    ...
    for (fname of AppCache) {
        if (cfile = await caches.match(fname)) {
            if (fname.endsWith('.js') || fname.endsWith('.css') || fname.endsWith('.json') || fname.endsWith('.txt') || fname.endsWith('.html') || fname.endsWith('.csv') || fname.includes('fonts')) {
                ...
                (cfile.toString() != r.toString()) ? nf.push(fname) : null;
            } else {
                ...
                (cfile.size !== parseInt(r)) ? nf.push(fname) : null;
...
self.addEventListener('push', function (event) {
    ...
    if (event && event.data) {
        ...
        const title = (self.pushData.title !== '') ? self.pushData.title : 'Notification';
        let who = event.data.json();
        let xzz = JSON.parse(who.body)
        const options = {
            // body: self.pushData.body
            body: 'Connecting to ' + xzz.who
        };

        event.waitUntil(self.registration.showNotification(title, options));
        notification_in = event.data.json();
        northBridge({ req: "pushin", data: event.data.text() });
    }
});

// Notification click event listener
self.addEventListener('notificationclick', e => {
    let url = location.href.replace('sw.js', '');
    let urll = url + 'talking';
    console.log(urll);
    e.notification.close();
    e.waitUntil(clients.matchAll({ type: 'window' }).then(clientsArr => {
        const hadWindowToFocus = clientsArr.some(windowClient => windowClient.url === url ? (windowClient.focus(), true) : false);
        if (!hadWindowToFocus) clients.openWindow(urll).then(windowClient => windowClient ? windowClient.focus() : null);
    }));
});
